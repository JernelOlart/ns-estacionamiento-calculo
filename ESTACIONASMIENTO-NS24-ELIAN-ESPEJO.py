...
def crear_plano_estacionamiento():
    ...
    for i in range(num_espacios):
        ...
        rectangulo = np.array([
            [largo_total - x, 0],
            [largo_total - (x + ancho_espacio * np.cos(np.radians(angulo_inclinacion))), ancho_espacio * np.sin(np.radians(angulo_inclinacion))],
            [largo_total - (x + ancho_espacio * np.cos(np.radians(angulo_inclinacion)) - alto_espacio * np.sin(np.radians(angulo_inclinacion))),
             alto_espacio * np.cos(np.radians(angulo_inclinacion)) + ancho_espacio * np.sin(np.radians(angulo_inclinacion))],
            [largo_total - (x - alto_espacio * np.sin(np.radians(angulo_inclinacion))), alto_espacio * np.cos(np.radians(angulo_inclinacion))]
        ])
        ax.add_patch(plt.Polygon(rectangulo, closed=True, fill=None, edgecolor='b'))

        # Etiqueta numérica para cada espacio
        etiqueta = str(i + 1)
        ax.text(largo_total - (x + ancho_espacio / 2 * np.cos(np.radians(angulo_inclinacion))), 
    ...
    # ax.set_title('Plano del Estacionamiento Núcleo Sepec', pad=40)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('plano_estacionamiento_espejo.png')
    plt.show()
